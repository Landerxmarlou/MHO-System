    ...
    if (!canAccessBarangay($barangayId) || $periodId <= 0 || $programId <= 0) {
        ...
        if ($existing) {
            if (!isset($_POST['force_create'])) {
                ...
                $stmt = $db->prepare(
                    'INSERT INTO report_submission (barangay_id, period_id, program_id, status, submitted_by) VALUES (?, ?, ?, ?, ?)'
                );
                $stmt->execute([$barangayId, $periodId, $programId, 'draft', (int) $_SESSION['user_id']]);
