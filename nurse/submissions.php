...
if (!empty($barangayIds)) {
    ...
    $params = array_merge($barangayIds, [$userId]);

    $where = "rs.barangay_id IN ($placeholders) AND rs.submitted_by = ?";
    ...
    $stmt = $db->prepare(
        "SELECT rs.id, rs.report_code, b.name AS barangay, hp.name AS program, hp.code AS program_code,
    ...
    <div class="table-responsive">
        <table class="table table-hover mb-0">
            <thead class="table-light">
                <tr>
                    <th>Report Code</th><th>Period</th><th>Barangay</th><th>Program</th><th>Status</th><th>Updated</th><th></th>
                </tr>
            </thead>
            <tbody>
