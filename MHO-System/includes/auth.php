...
{
    $stmt = $db->prepare(
        'SELECT id, username, email, password_hash, role, full_name, position, is_active
...
{
    $stmt = $db->prepare(
        'SELECT rs.*, b.name AS barangay_name, hp.code AS program_code, hp.name AS program_name,
